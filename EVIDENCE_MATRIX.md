...
| A11 | Live Arch Claude-session ingestion/indexing | 2026-04-04 / verified 2026-05-12 | Claude session artifacts were large, scattered, and hard to inspect directly. | Codex read a large Claude session, wrote a sanitized summary, and generated an index over 41 Claude session JSONLs. | `LIVE_SWEEP_2026-05-12.md` | Codex became the audit/indexing layer over Claude’s own session history on Arch. | Ready | Exact paths, session IDs, private transcript content | Publish task-level summary only. |
| D1 | Debian/Zorin RDP 0x207 handoff | 2026-04-26 | Claude/Lara escalated a concrete remote-desktop failure to Codex after server-side checks did not close the problem. | Codex operated from the Mac/Debian state and the user later recorded a working result without reboot. | `LIVE_SWEEP_2026-05-12.md` | Codex turned a Claude handoff into live workstation repair work. | Needs excerpt | Passwords, IPs, hostnames, user identity details | Extract only problem/action/success marker, fully redacted. |
| D2 | Debian/Zorin OpenClaw migration | 2026-04-27 | Agent infrastructure needed to move from Arch to Debian/Zorin and the user rejected package-level shortcuts. | Codex followed the hackable-source requirement and worked through the agent copy/setup path. | `LIVE_SWEEP_2026-05-12.md` | Codex acted as the cross-machine agent-infra repair layer. | Needs excerpt | Bot tokens, bot names, private chat content | Extract only source requirement, migration action, verification marker. |
| D3 | Debian OpenClaw model/provider failure loop | 2026-04-27 onward | OpenClaw provider/model routing produced hard errors and runtime confusion. | Codex history shows provider/model debugging around HTTP 400/401-style failures and runtime config distinctions. | `LIVE_SWEEP_2026-05-12.md`, `DEBIAN_OPENCLAW_FAILURES.md` | Codex debugged model-provider failures in the Claude-adjacent agent stack. | Needs excerpt | API keys, model account names, tokens | Use redacted error strings only. |
| D4 | Debian mirror of Arch Claude-session ingestion/indexing | 2026-04-04 / mirrored on Debian | Claude session artifacts were too large/sprawled for manual review. | Debian Codex memory also contains the task record for reading a large Claude session and generating a session index. | `LIVE_SWEEP_2026-05-12.md` | The Codex audit trail persisted across machines, not only on Arch. | Ready | Exact paths, session UUIDs | Publish task-level summary; keep raw session IDs out. |
| W1 | Windows Codex handoff for Claude/Lara recovery | 2026-04-23 / verified 2026-05-12 | Claude-side session recovery had become confused across roots, binaries, and continuity candidates. | Codex wrote a handoff for Claude that separated origin session state from surviving continuity state, documented misses, and identified runtime/root mismatch as the test variable. | `LIVE_SWEEP_2026-05-12.md` | Windows Codex functioned as a forensic operator for Claude's own session recovery problem. | Ready | Windows paths, volume IDs, session UUIDs, private transcript phrases | Publish the description, not raw paths or transcript lines. |
